...
vcf_path = '/Users/nathan/OneDrive - Georgia Institute of Technology/PhD/2025 Spring/Adv Programming in the Health Sciences/Module_4/YHPedigree_Class.vcf'    # modify this line if needed to match your file location
...
converted_genotype = allel.GenotypeArray(genotype) #Genotype array with parents as first two columns
print(converted_genotype)
print(converted_genotype.shape)
...
# print("paternal_genotype: ")
# print(paternal_genotype)
# print(paternal_genotype.shape)
# print(type(paternal_genotype))
# print("maternal_genotype: ")
# print(maternal_genotype)
...
# Replace "None" with your code
maternal_mask = (paternal_genotype.is_hom()) & (maternal_genotype.is_het())
...
print("paternal masked genotype shape is ", paternal_masked_genotype.shape)
...
print(paternally_painted.shape)
"""
...
paternally_painted[~paternal_phased[:, 2:].is_phased] = np.nan #select offspring samples only
maternally_painted[~maternal_phased[:, 2:].is_phased] = np.nan
...
paternally_painted[depth[paternal_mask][:, 2:] < 5] = np.nan # depth array also need to be masked correspondly
maternally_painted[depth[maternal_mask][:, 2:] < 5] = np.nan
...
for chrom in unique_chromosomes:
    ...
    # Replace "None" with your code
    chromosome_pat = chromosome[paternal_mask]
    paternal_slice = paternally_painted[chromosome_pat== chrom]
    ...
    # step
    chromosome_mat = chromosome[maternal_mask]
    maternal_slice = maternally_painted[chromosome_mat == chrom]
    mat_means = np.nanmean(maternal_slice, axis =0)
    all_maternal_means.append(mat_means)

# print(all_paternal_means)
# print(all_maternal_means)
...
Your answer: With the results, I think the sex-determining locus is on LG10 because in paternal inheritance figure,
there is a huge difference between male LG10 and female LG10 without any overlapping region. 
In that case, it should be inherited from father (paternal inheritance)
"""
